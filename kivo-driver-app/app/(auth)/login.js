...
export default function LoginScreen() {
  ...
  const handleLogin = async () => {
    if (!email.trim() || !password) { Alert.alert('Missing info', 'Enter email and password.'); return; }
    setLoading(true);
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          <View style={styles.brand}>
            ...
            <Text style={styles.brandText}>Kivo <Text style={{ color: theme.primary }}>Driver</Text></Text>
          </View>
          ...
          <View style={styles.field}>
            ...
            <TextInput style={styles.input} value={email} onChangeText={setEmail} placeholder="you@example.com" placeholderTextColor={theme.textFaint} keyboardType="email-address" autoCapitalize="none" editable={!loading} />
          </View>
          <View style={styles.field}>
          ...
          <View style={styles.footer}>
            ...
            <TouchableOpacity onPress={() => router.push('/(auth)/signup')}><Text style={styles.footerLink}>Apply now</Text></TouchableOpacity>
          </View>
        </ScrollView>
