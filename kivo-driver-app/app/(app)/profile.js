  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: d } = await supabase
        .from("drivers")
        .select(
          "full_name, phone_number, email, driver_category, driver_license_no, rating_average, rating_count, city:cities(city_name), vehicles(vehicle_type, plate_number, brand, model, color)",
        )
        .eq("user_id", user.id)
        .maybeSingle();
      setDriver(d);
          ...
          {driver.rating_count > 0 && (
            <View style={styles.ratingRow}>
              ...
              <Text style={styles.ratingCount}>
                ({driver.rating_count} rating
                {driver.rating_count !== 1 ? "s" : ""})
              </Text>
            </View>
          )}
        </View>
...
const styles = StyleSheet.create({
  ...
  ratingText: { color: theme.text, fontSize: 15, fontWeight: "800" },
  ratingCount: { color: theme.textMuted, fontSize: 12 },
  walletCard: {
    backgroundColor: theme.surface,
    borderRadius: 16,
    padding: 20,
