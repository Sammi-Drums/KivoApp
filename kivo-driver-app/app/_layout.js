...
export default function RootLayout() {
  ...
  const checkApproval = useCallback(async (userId) => {
    ...
    const { data, error } = await supabase
      ...
      .maybeSingle();
    if (error || !data) {
      setHasDriverRecord(false);
      setDriverId(null);
      setApprovalStatus(null);
      return;
    }
    setHasDriverRecord(true);
    setDriverId(data.id);
  ...
  useEffect(() => {
    ...
    const {
      ...
      if (session?.user) await checkApproval(session.user.id);
        ...
        setApprovalStatus(null);
        setDriverId(null);
        setHasDriverRecord(null);
      }
    });
    return () => subscription.unsubscribe();
  }, [checkApproval]);

  // Register for push notifications once we have an approved driver
  useEffect(() => {
    if (driverId && approvalStatus === "approved") {
      registerForPushNotifications(driverId);
    }
  }, [driverId, approvalStatus]);
