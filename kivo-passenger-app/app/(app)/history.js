...
export default function HistoryScreen() {
  ...
  const [ratedTripIds, setRatedTripIds] = useState(new Set());
  ...
  const load = useCallback(async () => {
    ...
    if (passenger) {
      ...
      // Which trips are already rated?
      const { data: rated } = await supabase
        .from("ratings")
        .select("trip_id")
        .eq("passenger_id", passenger.id);
      setRatedTripIds(new Set((rated || []).map((r) => r.trip_id)));
    }
    setLoading(false);
    setRefreshing(false);
  }, []);
        ...
        {trips.length === 0 ? (
          ...
          trips.map((trip) => {
            ...
            const canRate =
              ...
              trip.driver_id &&
              !ratedTripIds.has(trip.id);
            const alreadyRated = ratedTripIds.has(trip.id);
            return (
              <View key={trip.id} style={styles.card}>
                ...
                {canRate && (
                  ...
                  >
                    <Ionicons name="star" size={16} color={theme.warn} />
...
const styles = StyleSheet.create({
  ...
  rateBtn: {
    ...
    justifyContent: "center",
    gap: 6,
    backgroundColor: theme.warnFaint,
    borderWidth: 1,
  ...
  rateText: { color: theme.warn, fontSize: 14, fontWeight: "700" },
  ratedTag: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "center",
  ...
  ratedText: { color: theme.primary, fontSize: 13, fontWeight: "600" },
});
